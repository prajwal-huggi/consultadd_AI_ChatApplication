...
export class OpenAiResponseHandler {
  ...
  /*
    when the chatgpt is sending the message it is sending in the form 
    of the chunks so we are storing how many chunks are delivered
    */
  private run_id = "";
  ...
  constructor(
    ...
    /*
            the above line of code is used so that when the openAi server is sending the data continuously to the 
            nodejs server. At the very last when it will come to know that when it is the last bit then in that case the above
            function is used.
        */
  }

  run = async () => {};// It is just an orchestration
  dispose = async () => {
    if(this.is_done){
        return ;
    }
